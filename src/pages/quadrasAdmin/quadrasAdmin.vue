    ...
    <div class="q-pa-md">
        <q-table title="Cadastro de Quadras" :rows="rows" :columns="columns" row-key="name" :rows-per-page-options="[0]"
            ...
            <template v-slot:body="props">
                <q-tr :props="props">
                    <q-td auto-width>
                        <q-btn size="md" color="green" dense @click="this.$router.push('/admin/quadras/edit/' + props.row.id)" icon="edit" />
    ...
    methods: {
        async apagarRegistro(id) {
            if(confirm('Deseja apagar este registro?')) {
                await this.$gf.executeMethod('DELETE', '/sportsCourt/' + id)
                this.rows = (await this.$gf.executeMethod('GET', '/sportsCourt')).data.sportsCourts
            }
        },
    },
    async mounted() {
